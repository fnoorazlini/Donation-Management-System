...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Fetch donation allocation report data from the database with filter applied
    $sql = "SELECT 
                r.ReportID,
                r.ReportName,
                r.ReportDate,
            ...
            " . $whereClause . "
            AND r.ReportName LIKE '%allocation%'
            ORDER BY r.ReportDate DESC
            LIMIT $start_from, $records_per_page";
    $result = $dbCon->query($sql);
} else {
    // Fetch all donation allocation reports if no filters are applied initially
    $sql = "SELECT 
                r.ReportID,
                r.ReportName,
                r.ReportDate,
            ...
            FROM report r
            LEFT JOIN clerk c ON r.ClerkID = c.ClerkID
            WHERE r.ReportName LIKE '%allocation%'
            ORDER BY r.ReportDate DESC
            LIMIT $start_from, $records_per_page";
    $result = $dbCon->query($sql);
}
...
$total_records_sql = "SELECT COUNT(*) AS total FROM report r LEFT JOIN clerk c ON r.ClerkID = c.ClerkID WHERE r.ReportName LIKE '%allocation%'";
if (!empty($searchTerm)) {
...
<head>
    ...
    <title>Donation Allocation Report</title>
    ...
    <div class="container">
        ...
        <div class="pagination">
            ...
            for ($i = 1; $i <= $total_pages; $i++) {
                echo "<a href='allocationReport.php?page=" . $i . "'";
                if ($i == $page) {
                    echo " class='active'";
                }
                echo ">" . $i . "</a>";
